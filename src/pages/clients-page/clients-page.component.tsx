...
class ClientsPage extends Component<{}, ClientsPageProps> {
    ...
    loadClientsPageData = async () => {
        ...
        if (!this.state.isLoading) {
            var { visits: pharmVisits, total: totalPharm } = await this.visitService.getAllVisitsPaginated(1, 5, '', ClientType.pharmacy);
            var { visits: docVisits, total: totalDoc } = await this.visitService.getAllVisitsPaginated(1, 5, '', ClientType.doctor);
            var { visits: wholeVisits, total: totalWhole } = await this.visitService.getAllVisitsPaginated(1, 5, '', ClientType.wholesaler);
            this.setState({ isLoading: false, hasData: true, pharmVisits: pharmVisits, totalPharm: totalPharm, docVisits: docVisits, totalDoc: totalDoc });
        }
    };
    ...
    handlePharmVisitsFilter = async () => {
        ...
        var { visits: pharmVisits, total: totalPharm } = await this.visitService.getAllVisitsPaginated(1, this.state.sizePharm, this.state.pharmSearchText, ClientType.pharmacy);
    ...
    handleDocVisitsFilter = async () => {
        ...
        var { visits: docVisits, total: totalDoc } = await this.visitService.getAllVisitsPaginated(1, this.state.sizeDoc, this.state.docSearchText, ClientType.doctor);
    ...
    handlePharmPageChange = async (page: number) => {
        ...
        var { visits: pharmVisits, total: totalPharm } = await this.visitService.getAllVisitsPaginated(page, this.state.sizePharm, this.state.pharmSearchText, ClientType.pharmacy);
        this.setState({ pharmVisits: pharmVisits, totalPharm: totalPharm, loadingVisitsData: false, });
    }

    handleDocPageChange = async (page: number) => {
        this.setState({ loadingVisitsData: true, docPage: page });
        var { visits: docVisits, total: totalDoc } = await this.visitService.getAllVisitsPaginated(page, this.state.sizeDoc, this.state.docSearchText, ClientType.doctor);
        this.setState({ docVisits: docVisits, totalDoc: totalDoc, loadingVisitsData: false, });
    }

    handlePharmRowNumChange = async (size: number) => {
        this.setState({ loadingVisitsData: true, pharmPage: 1, sizePharm: size });
        var { visits: pharmVisits, total: totalPharm } = await this.visitService.getAllVisitsPaginated(1, size, this.state.pharmSearchText, ClientType.pharmacy);
        this.setState({ pharmVisits: pharmVisits, totalPharm: totalPharm, loadingVisitsData: false, });
    }

    handleDocRowNumChange = async (size: number) => {
        this.setState({ loadingVisitsData: true, docPage: 1, sizeDoc: size });
        var { visits: docVisits, total: totalDoc } = await this.visitService.getAllVisitsPaginated(1, size, this.state.docSearchText, ClientType.doctor);
        this.setState({ docVisits: docVisits, totalDoc: totalDoc, loadingVisitsData: false, });
    }
